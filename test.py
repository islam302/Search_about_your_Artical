...
#
#         self.start_driver()  # Move this line here to start the driver before using it
#         for word in words:
...
# import urllib.parse
#
# def search_articles(search_term, url="https://search.yahoo.com/search?p="):
#     found_links = []
#     response = requests.get(url + urllib.parse.quote(search_term))
#     if response.status_code == 200:
#         soup = BeautifulSoup(response.content, "html.parser")
#         links = soup.find_all("a")
#         for link in links:
...
# def search_articles(search_term, base_url):
#     found_links = []
#     response = requests.get(base_url + urllib.parse.quote(search_term, safe=''))
#     if response.status_code == 200:
#         soup = BeautifulSoup(response.content, "html.parser")
#         results = soup.find_all('a', class_='ac-algo')
#         for result in results:
...
#
# search_term = "غزة"
# yahoo_base_url = "https://www.bing.com/search?p="
#
# results_yahoo = search_articles(search_term, yahoo_base_url)
# print("Yahoo search results:")
...
def search_articles(url="https://www.bing.com/search?q=%D8%BA%D8%B2%D8%A9"):
    found_links = []
    ...
    if response.status_code == 200:
        ...
        for link in links:
            href = link.get("href")
            if href.startswith("/url?q="):
                found_links.append(href.replace("/url?q=", "").split("&sa=")[0])
                links_found += 1
                if links_found >= 10:
                    break
        return found_links
